...
CREATE TABLE users (
    ...
    profile_text TEXT DEFAULT 'よろしくお願いします。',
    favorite_id BIGINT,
...
CREATE TABLE posts (
    ...
    ruby_content JSONB, 
    genre_id BIGINT REFERENCES genres(id), -- 外部キー制約を追加
    weekly_theme_id BIGINT REFERENCES weekly_themes(id) ON DELETE SET NULL,
...
-- ★追加: 通知機能
-- MySQLのENUMの代わりにCHECK制約を使用します
-- Node.jsコードに合わせて 'reply' も許可リストに入れています
CREATE TABLE notifications (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id BIGINT REFERENCES users(id) ON DELETE CASCADE, -- 通知を受け取る人
    from_user_id BIGINT REFERENCES users(id) ON DELETE CASCADE, -- 通知の発生源の人
    type TEXT CHECK (type IN ('like', 'follow', 'fumi', 'reply')) NOT NULL,
